...
public class CreateHandler : MongoDataAccess<User>, ICommand<User>
{
    #region Variable
    ...
    #region Constructor
    public CreateHandler(SQLConfig sqlConfig) : base(sqlConfig)
    {
    ...
    public async Task<User> CommandHandlerAsync(User model)
    {
        ...
        try
        {
            ...
            //3. Create the user by using clients connection string.
            ...
            #region 3. Create the user by using clients connection string.

            INoSQLBaseAccess<User> baseUserAccess = new MongoDataAccess<User>(userConfig);
            model.PasswordHash = BCrypt.Net.BCrypt.HashPassword(model.PasswordHash);
            var savedUser = await baseUserAccess.CreateAsync(model);
            model = savedUser;
            #endregion
        }
        catch (Exception ex)
