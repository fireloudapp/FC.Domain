...
public class UpdateHandler: MongoDataAccess<User>, ICommand<User>
{
    #region Variable
    ...
    #region Constructor
    public UpdateHandler(SQLConfig sqlConfig) : base(sqlConfig)
    {
    ...
    public async Task<User> CommandHandlerAsync(User model)
    {
        ...
        try
        {
            ...
            //3. Update the user by using clients connection string.
            ...
            #region 3. Update the user by using clients connection string.

            INoSQLBaseAccess<User> baseUserAccess = new MongoDataAccess<User>(userConfig);
            model.PasswordHash = BCrypt.Net.BCrypt.HashPassword(model.PasswordHash);
            await baseUserAccess.ModelCollection
                .ReplaceOneAsync(filter:u => u.Id == model.Id,
                    model);
